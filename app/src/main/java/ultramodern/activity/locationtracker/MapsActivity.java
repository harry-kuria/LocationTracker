    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                try {
                    String databaseListLatitude = snapshot.child("Latitude").getValue().toString().substring(1,snapshot.child("Latitude").getValue().toString().length()-1);
                    String databaseListLongitude = snapshot.child("Latitude").getValue().toString().substring(1,snapshot.child("Longitude").getValue().toString().length()-1);

                    String[] stringLat = databaseListLatitude.split(",");
                    Arrays.sort(stringLat);
                    String latitude = stringLat[stringLat.length-1].split("=")[1];

                    String[] stringLong = databaseListLongitude.split(",");
